...
if (isset($_POST['commentid'])&&!empty($_POST['commentid'])) {
	...
	if ($numrows==0) {
		...
		$exec = $db->exec("INSERT INTO reactcomment VALUES($commentid, $reacterid, 0);");
		if ($exec)
			echo 1;
		else echo 0;
	}
		...
		if ($row['IsLike']==0) {
			...
			$query = $db->exec("UPDATE reactcomment SET IsLike=0 WHERE ReacterID = $reacterid AND CommentID = $commentid;");
